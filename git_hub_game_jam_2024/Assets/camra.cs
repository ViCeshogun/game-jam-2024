...
public class camra : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (time > 4&& time<6) 
        {
            bullet.transform.position = Vector3.MoveTowards(bullet.transform.position, bob_pos, 0.03f);
        ...
        if (time == 6) { 
            time = 0; 
            random_check = 0; }
        if (random == 1) { bob_pos = new Vector2(-11.5f, 0); }
        if (random == 2) { bob_pos = new Vector2(11.5f, 0); }
        if (random == 3) { bob_pos = new Vector2(0, 5.5f); }
        if (random == 4) { bob_pos = new Vector2(11.5f, 5.5f); }
        if (random == 5) { bob_pos = new Vector2(-11.5f, 5.5f); }
